class SilktideCookieBanner {
  /**
   * Membuat instance baru dari SilktideCookieBanner untuk mengelola banner dan modal persetujuan cookie.
   * @param {Object} config Konfigurasi untuk banner cookie, termasuk cookieTypes, teks, dan pengaturan tampilan.
   */
  constructor(config) {
    this.config = {
      cookieTypes: [], // Default ke array kosong jika tidak ada
      ...config,
    };
  ...
  /**
   * Membuat elemen wrapper DOM untuk menampung banner, modal, dan ikon cookie.
   * @returns {void}
   */
  createWrapper() {
  ...
  handleCookieChoice(accepted) {
    ...
    this.config.cookieTypes.forEach((type) => {
      if (type.required) {
        ...
        if (accepted && typeof type.onAccept === 'function') type.onAccept();
        else if (!accepted && typeof type.onReject === 'function') type.onReject();
      }
    });
  ...
  /**
   * Memperbarui status checkbox di modal preferensi berdasarkan localStorage atau menyimpan status ke localStorage.
   * @param {boolean} [saveToStorage=false] Jika true, simpan status checkbox ke localStorage; jika false, baca dari localStorage.
   * @returns {void}
   */
  ...
  /**
   * Menghasilkan konten HTML untuk modal preferensi cookie.
   * @returns {string} Konten HTML untuk modal.
   */
  getModalContent() {
    ...
    const savePreferencesButton = `<button class="save-preferences st-button">${savePreferencesText}</button>`;
  ...
  /**
   * Menyiapkan event listener untuk elemen interaktif di modal preferensi (tombol tutup, terima, tolak, simpan).
   * @returns {void}
   */
  setupModalEventListeners() {
    ...
    addListener(savePreferencesButton, 'click', () => {
      ...
      this.config.cookieTypes.forEach((type) => {
        ...
        newChoices[type.id] = type事例 required ? true : checkbox.checked;
...
(function () {
   ...
   * Membuat instance SilktideCookieBanner jika belum ada dan menyiapkan observer untuk callback onConsentGiven.
   * @returns {void}
   */
  function initCookieBanner() {
    if (!cookieBanner) {
      if (!config.cookieTypes || !Array.isArray(config.cookieTypes)) {
        console.warn('Inisialisasi banner cookie ditunda: config.cookieTypes tidak valid. Panggil updateCookieBannerConfig dengan konfigurasi yang valid.');
        return;
      }
      ...
      if (typeof config.onConsentGiven === 'function') {
        ...
        const observer = new MutationObserver(() => {
          const consentKeys = [
            `silktideCookieChoice_analytics${bannerSuffix}`,
            `silktideCookieChoice_marketing${bannerSuffix}`,
            `silktideCookieChoice_necessary${bannerSuffix}`,
            `silktideCookieChoice_functionality${bannerSuffix}`
          ];
